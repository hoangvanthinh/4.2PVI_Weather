...
static uint8_t SerializedMACAddress[6] = {
...
static void readHoldingRegister(void) {
    ...
    if ((MODBUS_COMMAND.StartAddress.Val > ((int) HOLDING_REG_SIZE_42 - 1)) ||
            ((MODBUS_COMMAND.StartAddress.Val - 1 + MODBUS_COMMAND.NumberOfRegister.Val) > (int) HOLDING_REG_SIZE_42)) {
