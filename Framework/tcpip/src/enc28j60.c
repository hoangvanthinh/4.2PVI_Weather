 ...
 *                  Helpers.h
 *                  Delay.h
 * Processor:       PIC18F2xK22/24EP256MC202(I confirmed it only with this PIC)
 * Compiler:        MPLAB(R) XC8 C Compiler Version 1.40
 *                  MPLAB(R) XC16 C Compiler Version 1.30
 *                  XC32 is also supported but not confirmed
 ...
 * Author               Date        Comment
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * Howard Schlunder     6/28/04 OriginalChanged to support XC8 compiler
 ...
 * きむ茶工房(きむしげ)  4/20/17 Changed to support XC8 compiler
 *                              I used "microchip_solutions_v2013-06-15"
 * きむ茶工房(きむしげ)  5/29/17 The XC16 compiler also works
********************************************************************/
#define __ENC28J60_C
...
#define SRC ((0x7<<5) | 0x1F)   // システムリセットコマンドはアドレスを使用しません。
                                // 但し、0x1Fが必要です。

// データシートで指定された最大SPI周波数(通信速度)
#define ENC_MAX_SPI_FREQ    (20000000ul)    // Hz

#define ETHER_IP    (0x00u)
#define ETHER_ARP   (0x06u)

// ハードウェアによってすべてのRXフレームの先頭に付加されるヘッダ
...
void MACInit(void)
{
    ...
    if (SYS_CLK_FrequencyInstructionGet() >= 60000000ul) {
           ENC_SPICON1 = 0x0B;  // 1:1 primary prescale, 6:1 secondary prescale (10MHz @ 60MIPS)
    } else ENC_SPICON1 = 0x0F;  // 1:1 primary prescale, 5:1 secondary prescale (8MHz  @ 40MIPS)
    ...
    // ENC28J60全体をリセットし、すべてのレジスタをクリアします。
    // また、CLKRDYがセットされるのを待ちます。
    // ESTATのビット3は実装されていないビットです。
    // それが「1」と読むと、それは、部品がRESETにあるか、SPI接続に問題があることを意味します。
    // このループは、先に進む前にENC28J60と通信できることを確認します。
    do
    {
        SendSystemReset();
...
bool MACGetHeader(MAC_ADDR *remote, BYTE* type)
{
    ...
    // ENC28J60から返されたデータを検証します。
    // 通信中に単一のSPIビットエラーが発生したり、瞬間的な電源異常が発生すると、
    // まれにデータが壊れる可能性があります。
 ...
 *
 * 前提条件:        パケットは、MACPut()とMACPutHeader()を呼び出して作成されています。
 *
 * Input:           None
 *
 * Output:          None
 ...
 * Side Effects:    None
 *
 * Overview:        MACFlushは、現在のTXパケットをイーサネット媒体上に送出させる。
 *                  ハードウェアは、CRC生成、衝突再送信および他の詳細を制御および処理する。
 *
 * Note:            送信が完了すると（MACIsTxReady()はTRUEを返します）、
 *                  MACFlush()を再度呼び出す事によって、パケットを変更して再度送信する事ができます。
 *                  MACPutHeader()又はMACPut()が(TXデータエリアで)呼び出されるまで、
 *                  TXバッファ内のデータは破損しません。
 *****************************************************************************/
void MACFlush(void)
{
    // 送信エラーが以前に発生した場合は送信ロジックをリセットします。
    // これはシリコンエラッタの回避策です。
    BFSReg(ECON1, ECON1_TXRST);
    BFCReg(ECON1, ECON1_TXRST);
    BFCReg(EIR, EIR_TXERIF | EIR_TXIF);

    // 送信を開始させます。送信が完了すると(MACIsTxReady()はTRUEを返します）、
    // MACFlush()を再度呼び出すことによって、パケットを変更して再度送信できます。
    // MACPutHeader()が呼び出されるまで、TXバッファ内のデータは破損しません。
    BFSReg(ECON1, ECON1_TXRTS);

    // 改訂 B5およびB7のシリコンエラッタ回避策
    if(ENCRevID == 0x05u || ENCRevID == 0x06u)
    {
        ...
        if(ReadETHReg(EIR).EIRbits.TXERIF || (AttemptCounter >= 1000u))
        {
            ...
            // 送信がキャンセルされている場合は、直前の送信をキャンセルします。
            BFCReg(ECON1, ECON1_TXRTS);

            // 現在の読み取りポインタを保存する（アプリケーションによって制御される）
            ...
            // 遅くコリジョンが発生した場合に再送信を実装する
            // (これは、特定のリンクパルスが送信と同時に到着した時にB5で発生する可能性がある）
            for(i = 0; i < 16u; i++)
            {
...
{
    TCPIP_UINT16_VAL ReadSave = {0}, WriteSave = {0};
...
// with I2C temperature sensor on the same SPI wires
    ...
    while(i--)
    {
        if(((TCPIP_UINT8_VAL*)&Dummy)->bits.b0)
        {
            // End bust operation
            ENC_CS_IO = 1;
            ((TCPIP_UINT8_VAL*)&Dummy)->bits.b0 = 0;
 ...
 *
 * 前提条件:        SPIバスを初期化する必要があります（MACInit()で完了）。
 *
 * Input:           None
 *
 * Output:          None
 ...
 * Side Effects:    None
 *
 * Overview:        SendSystemResetは、システムリセットSPIコマンドをイーサネットコントローラに送信します。
 *                  全てのレジスタの内容をリセットし(ECOCONを除く)、デバイスを電源投入時のデフォルト状態に戻します。
 *
 * Note:            None
 *****************************************************************************/
static void SendSystemReset(void)
{
    ...
    // 注意：パワーセーブ機能によりリセットが実行されない場合がありますので、
    // リセットを発行する前にデバイスがパワーセーブ状態でない事を確認する必要があります。
    // パワーセーブ状態から復帰させる
    BFCReg(ECON2, ECON2_PWRSV);

    // レギュレータは正常なレギュレーションに達し、すべてのクロックが動作するまで待ちます。
    DelayMs(1);
    ...
    ClearSPIDoneFlag();
    ENC_SSPBUF = SRC;
    WaitForDataByte();      // コマンドが送信されるまで待ちます。
    Dummy = ENC_SSPBUF;
    ENC_CS_IO = 1;

    // オシレータのスタートアップタイマとPHYがレディになるのを待ちます。
    DelayMs(1);
}//end SendSystemReset
 ...
 *
 * PreCondition:    SPIバスを初期化する必要があります（MACInit()で完了）。
 *                  読み出すレジスタに対応してバンクセレクトビットを設定する必要があります。
 *
 ...
 *                  clocks.
 *
 * Note:            このルーチンは、BFCReg()およびBFSReg()関数とほぼ同じです。 
 *                  それはスピードを最大にするためには別です。
 *                  ReadETHReg/ReadMACReg()関数とは異なり、
 *                  WriteReg()は、任意のETHまたはMACレジスタに書き込むことができます。
 *                  PHYレジスタへの書き込みはWritePHYReg（）で行う必要があります。
 *****************************************************************************/
static void WriteReg(BYTE Address, BYTE Data)
{
    volatile BYTE Dummy;
	...
	#if (SYS_CLK_FrequencyInstructionGet() > 30000000)
		Nop();
		Nop();
	#endif
	#if (SYS_CLK_FrequencyInstructionGet() > 40000000)
		Nop();
		Nop();
	#endif
	#if (SYS_CLK_FrequencyInstructionGet() > 50000000)
		Nop();
		Nop();
	#endif
 ...
 *
 * 前提条件:         SPI bus must be initialized (done in MACInit()).
 *                  変更するレジスタに対応して、バンク選択ビットを設定する必要があります。
 *
 * Input:           変更するレジスタの5ビットアドレス、上位3ビットは0でなければなりません。
 *                ? ビットフィールドクリア操作で使用されるバイト。
 *
 * Output:          None
 *
 * Side Effects:    None
 *
 * Overview:        BFCRegは8ビットのBFCオペコード/アドレスバイトをSPIに送信し、
 *                  次にバイトデータを送信します。
 *
 * Note:            このルーチンは、WriteReg()およびBFSReg()関数とほぼ同じです。
 *                  スピードを最大限にするために別々です。
 *                ? BFCReg()はETHレジスタでのみ使用する必要があります。
 *****************************************************************************/
static void BFCReg(BYTE Address, BYTE Data)
{
    volatile BYTE Dummy;
    ...
    ENC_CS_IO = 0;
    ClearSPIDoneFlag();
    ENC_SSPBUF = BFC | Address; // オペコードとアドレスを送信します。
    WaitForDataByte();          // オペコード/アドレスが送信されるまで待ちます。
    Dummy = ENC_SSPBUF;
    ENC_SSPBUF = Data;          // 書き込まれるバイトを送信します。
    WaitForDataByte();          // レジスタが書き込まれるまで待ちます。
    Dummy = ENC_SSPBUF;
    ENC_CS_IO = 1;
}//end BFCReg
 ...
 *                  次にバイトデータを送信します。
 *
 * Note:            このルーチンは、WriteReg()およびBFCReg()関数とほぼ同じです。
 *                  スピードを最大限にするために別々です。
 *                  BFSReg()はETHレジスタでのみ使用する必要があります。
 *****************************************************************************/
static void BFSReg(BYTE Address, BYTE Data)
{
    volatile BYTE Dummy;
...
void WritePHYReg(BYTE Register, WORD Data)
{
    ...
    WriteReg((BYTE)MIWRL, ((TCPIP_UINT16_VAL*)&Data)->v[0]);
    WriteReg((BYTE)MIWRH, ((TCPIP_UINT16_VAL*)&Data)->v[1]);
 ...
 * 前提条件:        SPIバスを初期化する必要があります（MACInit()で完了）。
 *
 * Input:           バンク選択2ビットを含む上位バイトにアドレスを登録します。
 *
 * Output:          None
 *
 * Side Effects:    None
 *
 * Overview:        BankSelはレジスタアドレスの上位バイトを取り、
 *                  ETHCON1のバンク選択ビットを一致させるように変更します。
 *
 * Note:            None
 *****************************************************************************/
static void BankSel(WORD Register)
...
{
    DTCPIP_UINT16_VAL CRC = {0xFFFFFFFF};
    BYTE HTRegister;
    BYTE i, j;
