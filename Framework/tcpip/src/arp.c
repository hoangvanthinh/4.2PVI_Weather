...
bool ARPProcess(void)
{
    ...
    switch (smARP) {
    case SM_ARP_IDLE:
        //printf("Obtain the incoming ARP packet\r\n");
        ...
        // Handle incoming ARP requests for our MAC address: TargetMACAddr
        if (packet.Operation == ARP_OPERATION_REQ) {
            if (packet.TargetIPAddr.Val != AppConfig.MyIPAddr.Val) {
            //if (packet.TargetMACAddr != AppConfig.MyMACAddr) {
                return true;
            }
#ifdef STACK_USE_ZEROCONF_LINK_LOCAL
